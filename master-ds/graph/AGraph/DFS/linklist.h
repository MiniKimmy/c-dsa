#define REFERENCEELEMENT 0  //0表示值类型value,1表示引用类型value

#if USEAGRAPH ==1
typedef int LINKELEMENT;
#endif // USEAGRAPH ==1
...
int add(SLinkList* L, LINKELEMENT value);
LINKELEMENT* getValueSLinkList(SLinkList* L, int index);
int setValueSLinkList(SLinkList * L, int index, LINKELEMENT value);
int insert(SLinkList* L, int index, LINKELEMENT value);
int removeAt(SLinkList* L, int index);
int hasElement(SLinkList* L, LINKELEMENT value);
...
int clearSLinkList(SLinkList* L)
{
    ...
    for (int i = 0; i < L->count; i++) {
        free((ArcNode*)arr[i]);
#if REFERENCEELEMENT == 1
        free(((ArcNode*)arr[i])->v);
#endif
        arr[i] = NULL;
    }
